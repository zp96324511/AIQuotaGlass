...
### 1.3 品牌标识
- 标志名称:「Quota Gauge」,用玻璃方牌承载一个缺口式配额环,中心三根柱体对应 5 小时、本周、本月三个用量窗口。
- AI 识别点:左上角四角光芒表示模型推理,右上角青绿色光点表示实时刷新。
- 主色阶:玻璃深底 `#101a2a` / `#171529`,状态渐变 `#4cd6a4` → `#5aa7ff`,月度暖色 `#ed9941`。
- 材质规则:深色底、双层细描边、左上方玻璃反光;缩小到 16px 托盘尺寸时以高亮配额环和尾部为主识别,三柱作为大尺寸标志的补充细节。
- 使用范围:标题栏、设置弹窗、Windows/macOS 应用图标;前端统一引用 `frontend/public/aiquotaglass-logo.svg`,托盘使用 `build/trayicon-light.png` / `build/trayicon-dark.png` 简化变体,禁止再用纯色圆点代替品牌标志。
...
主窗口以透明玻璃卡片展示多个厂商的用量窗口、重置倒计时和可选的使用明细。

![主窗口](images/widget.png)

设置弹窗集中维护刷新间隔、通知、贴边吸附、账号凭据、告警阈值和透明度。

![设置面板](images/settings.png)

贴边后窗口收缩为三条进度条，分别对应 5 小时、本周、本月用量，点击缩条可展开主窗口。

![贴边缩条](images/edge-docked.png)
